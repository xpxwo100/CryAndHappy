/*
 * Purl (A JavaScript URL parser) v2.3.1 Developed and maintanined by Mark
 * Perkins, mark@allmarkedup.com Source repository:
 * https://github.com/allmarkedup/jQuery-URL-Parser Licensed under an MIT-style
 * license. See
...
(function(factory) {
	...
	function parseUri(url, strictMode) {
		var str = decodeURI(url), res = parser[strictMode || false
				? 'strict'
				: 'loose'].exec(str), uri = {
			attr : {},
			param : {},
	...
	function parseString(str) {
		return reduce(String(str).split(/&|;/), function(ret, pair) {
			...
			var eql = pair.indexOf('='), brace = lastBraceInKey(pair), key = pair
					.substr(0, brace || eql), val = pair.substr(brace || eql,
					pair.length);
		...
		return {
			...
			segment : function(seg) {
				if (typeof seg === 'undefined') {
					...
					seg = seg < 0 ? this.data.seg.path.length + seg : seg - 1; // negative
																				// segments
																				// count
																				// from
																				// the
