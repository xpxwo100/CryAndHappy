...
(function($) {
	...
	/*
	 * Call Example var a = jQuery("#videoInfo").formToJson();
	 */
	$.fn.formToJson = function(semantic) {
		// hand off to jQuery.param for proper encoding
		// var para = jQuery.param(this.formToArray(semantic));
		// para = para.replace(new RegExp(/\&/g), '","');
		// para = para.replace(new RegExp(/\=/g), '":"');
		// para = para.replace(new RegExp(/\+/g), " ");
		// para = '{"' + para + '"}';
		// para=decodeURIComponent(para);
		// return para;
		var mObj = this.formToArray(semantic);
		var mArray = new Array();
		for (var i = 0; i < mObj.length; i++) {
			var mObjChild = mObj[i];
			mArray.push('"' + mObjChild.id + '":"' + mObjChild.value + '"');
		}
		return "{" + mArray.join(",") + "}";
	};
	/**
